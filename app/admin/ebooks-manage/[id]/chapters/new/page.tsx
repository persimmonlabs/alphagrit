...
import { getEbookById, createChapter, uploadChapterImage, generateSlug, type Ebook } from '@/lib/supabase/ebooks-client'
...
export default function NewChapterPage() {
  ...
  async function checkAdminAndLoad() {
    ...
    const maxChapter = Math.max(0, ...(data.chapters?.map(c => c.chapter_number) || []))
    setForm(prev => ({ ...prev, chapter_number: maxChapter + 1 }))
    setLoading(false)
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (chapter && imageFile) {
        const imageUrl = await uploadChapterImage(imageFile, chapter.id)
        if (imageUrl) {
          const { updateChapter } = await import('@/lib/supabase/ebooks-client')
          await updateChapter(chapter.id, { cover_image_url: imageUrl })
        }
      }

      router.push(`/admin/ebooks-manage/${ebookId}/chapters`)
    } catch (error) {
      alert('Failed to create chapter: ' + (error as Error).message)
    }
    ...
    <div className="min-h-screen bg-black text-white p-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold mt-2">Add New Chapter</h1>
          ...
          <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-6">
            ...
            <div className="flex items-start gap-6">
              ...
              <div className="flex-1">
                <Label htmlFor="chapter_image">Upload Chapter Image</Label>
                <Input
                  id="chapter_image"
                  type="file"
                  accept="image/*"
